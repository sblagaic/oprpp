...
public class ArrayIndexedCollection<T> implements List<T> {
	...
	private static class GetArrayElements<T> implements ElementsGetter<T> {
		 ...
		 */
		public GetArrayElements(ArrayIndexedCollection<T> col) {
	...
	public void insert(T value, int position) {
		if (position < 0 || position > this.size) {
			throw new IndexOutOfBoundsException("Index must be greater than zero and smaller or equal to array size");
		}
	...
	public ElementsGetter<T> createElementsGetter() {
		 return new GetArrayElements<T>(this);
	}
